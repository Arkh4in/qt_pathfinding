...
void Graph2D::Triangulate()
{
    ...
    while (m_Vertice.size() > 0)
    {
        ...
        for (Triangle2D& tri : m_Triangles)
        {
            if (tri.GetCircumscribedCircle().IsPointContained(p))
            {
                toRemove.push(tri);
                //must handle edges here
                m_Triangles.append(Triangle2D(point, tri.GetA(), tri.GetB()));
                m_Triangles.append(Triangle2D(point, tri.GetB(), tri.GetC()));
                m_Triangles.append(Triangle2D(point, tri.GetC(), tri.GetA()));
            }
        }
        while (toRemove.size() > 0)
        {
